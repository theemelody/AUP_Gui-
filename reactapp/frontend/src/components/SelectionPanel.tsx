...
const BUILDING_KEY_PRIORITY = [
  'const_type', 'refurbishment_type', 'detail', 'mapbox_type',
  'feature_year_start', 'feature_year_end', 'cea_use_type1',
  'type', 'height', 'min_height', 'estimated_floors',
];
...
interface SelectionPanelProps {
  ...
  confirmedSelection: SelectionState | null;
  activeSelection: SelectionState;
  handleConfirmSelection: () => void;
  handleResetSelection: () => void;
}

function getOrderedBuildingEntries(building: Building): [string, unknown][] {
...
function SelectionPanel({
  ...
  return (
    <section className="selected-list-panel" aria-label="Selected buildings">
      <div>
        <button onClick={() => setIsOpened((v) => !v)} type="button" className="action-link">
      ...
      <Collapse isOpened={isOpened}>
        ...
        <div className="selected-list-body">
          ...
          {activeSelection.buildings.length === 0 ? (
            <div className="selected-empty">No buildings selected yet.</div>
          ) : (
            activeSelection.buildings.map((building, index) => (
              <article className="building-card" key={index}>
                ...
                <div className="building-props">
                  {getOrderedBuildingEntries(building as Building).map(([key, value]) => (
